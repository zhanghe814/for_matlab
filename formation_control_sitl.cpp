...
//20200327byzh
int start_flag=0;
Eigen::Vector3d pos_drone_uav0_start(0,0,0);
Eigen::Vector3d pos_drone_uav1_start(0,0,0);
Eigen::Vector3d pos_drone_uav2_start(0,0,0);
Eigen::Vector3d pos_drone_uav3_start(0,0,0);
//20200327byzh
...
//20200327byzh
Eigen::Vector3d pos_drone_uav0_matlab;
Eigen::Vector3d vel_drone_uav0_matlab;

Eigen::Vector3d pos_drone_uav1_matlab;
Eigen::Vector3d vel_drone_uav1_matlab;
Eigen::Vector3d pos_drone_uav2_matlab;
Eigen::Vector3d vel_drone_uav2_matlab;
Eigen::Vector3d pos_drone_uav3_matlab;
Eigen::Vector3d vel_drone_uav3_matlab;

void uav0_matlab_position_sub_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    pos_drone_uav0_matlab  = Eigen::Vector3d(msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);
    //cout<<"subscribe success"<<endl;
    //ROS_INFO("pos_drone_uav1_matlab: x:%0.6f, y:%0.6f, z:%0.6f", msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);
}
void uav0_matlab_velocity_sub_cb(const geometry_msgs::TwistStamped::ConstPtr &msg)
{
    vel_drone_uav0_matlab = Eigen::Vector3d(msg->twist.linear.x,msg->twist.linear.y,msg->twist.linear.z);
}

void uav1_matlab_position_sub_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    pos_drone_uav1_matlab  = Eigen::Vector3d(msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);
    //cout<<"subscribe success"<<endl;
    //ROS_INFO("pos_drone_uav1_matlab: x:%0.6f, y:%0.6f, z:%0.6f", msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);
}
void uav1_matlab_velocity_sub_cb(const geometry_msgs::TwistStamped::ConstPtr &msg)
{
    vel_drone_uav1_matlab = Eigen::Vector3d(msg->twist.linear.x,msg->twist.linear.y,msg->twist.linear.z);
}
void uav2_matlab_position_sub_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    pos_drone_uav2_matlab  = Eigen::Vector3d(msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);
    
}
void uav2_matlab_velocity_sub_cb(const geometry_msgs::TwistStamped::ConstPtr &msg)
{
    vel_drone_uav2_matlab = Eigen::Vector3d(msg->twist.linear.x,msg->twist.linear.y,msg->twist.linear.z);
}
void uav3_matlab_position_sub_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    pos_drone_uav3_matlab  = Eigen::Vector3d(msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);
    
}
void uav3_matlab_velocity_sub_cb(const geometry_msgs::TwistStamped::ConstPtr &msg)
{
    vel_drone_uav3_matlab = Eigen::Vector3d(msg->twist.linear.x,msg->twist.linear.y,msg->twist.linear.z);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber uav0_matlab_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav0_matlab/mavros/local_position/pose", 100, uav0_matlab_position_sub_cb);
    ros::Subscriber uav0_matlab_velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>("/uav0_matlab/mavros/local_position/velocity_local", 100, uav0_matlab_velocity_sub_cb);

    ros::Subscriber uav1_matlab_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav1_matlab/mavros/local_position/pose", 100, uav1_matlab_position_sub_cb);
    ros::Subscriber uav1_matlab_velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>("/uav1_matlab/mavros/local_position/velocity_local", 100, uav1_matlab_velocity_sub_cb);
    ros::Subscriber uav2_matlab_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav2_matlab/mavros/local_position/pose", 100, uav2_matlab_position_sub_cb);
    ros::Subscriber uav2_matlab_velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>("/uav2_matlab/mavros/local_position/velocity_local", 100, uav2_matlab_velocity_sub_cb);
    ros::Subscriber uav3_matlab_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav3_matlab/mavros/local_position/pose", 100, uav3_matlab_position_sub_cb);
    ros::Subscriber uav3_matlab_velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>("/uav3_matlab/mavros/local_position/velocity_local", 100, uav3_matlab_velocity_sub_cb);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主  循  环<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        ...
        case command_to_mavros::Idle:
            ...
            pos_setpoint0.type_mask = 0x4000;
            ...
            {
                uav0_mode_cmd.request.custom_mode = "OFFBOARD";
        ...
        case command_to_mavros::Move_ENU:
            ...
            // pos_sp[0] = pos_drone_uav1_matlab[0];//Eigen::Vector3d(Command_Now.Reference_State.position_ref[0],Command_Now.Reference_State.position_ref[1],Command_Now.Reference_State.position_ref[2]);
            // pos_sp[1] = pos_drone_uav1_matlab[1];
            // pos_sp[2] = pos_drone_uav1_matlab[2];
            ...
            pos_setpoint0.position.x = pos_drone_uav0_matlab[0];//pos_sp[0] +delta_uav1[0];
            pos_setpoint0.position.y = pos_drone_uav0_matlab[1];//pos_sp[1] +delta_uav1[1];
            pos_setpoint0.position.z = pos_drone_uav0_matlab[2];//pos_sp[2] +delta_uav1[2];

            pos_setpoint1.position.x = pos_drone_uav1_matlab[0];//pos_sp[0] +delta_uav1[0];
            pos_setpoint1.position.y = pos_drone_uav1_matlab[1];//pos_sp[1] +delta_uav1[1];
            pos_setpoint1.position.z = pos_drone_uav1_matlab[2];//pos_sp[2] +delta_uav1[2];

            pos_setpoint2.position.x = pos_drone_uav2_matlab[0];//pos_sp[0] +delta_uav2[0];
            pos_setpoint2.position.y = pos_drone_uav2_matlab[1];//pos_sp[1] +delta_uav2[1];
            pos_setpoint2.position.z = pos_drone_uav2_matlab[2];//pos_sp[2] +delta_uav2[2];

            pos_setpoint3.position.x = pos_drone_uav3_matlab[0];//pos_sp[0] +delta_uav3[0];
            pos_setpoint3.position.y = pos_drone_uav3_matlab[1];//pos_sp[1] +delta_uav3[1];
            pos_setpoint3.position.z = pos_drone_uav3_matlab[2];//pos_sp[2] +delta_uav3[2];
            ...
            uav0_setpoint_raw_local_pub.publish(pos_setpoint1);
            uav1_setpoint_raw_local_pub.publish(pos_setpoint1);
            uav2_setpoint_raw_local_pub.publish(pos_setpoint2);
            uav3_setpoint_raw_local_pub.publish(pos_setpoint3);*/

            ///xy为速度控制，z为位置控制
            pos_setpoint0.type_mask = 0b100111000011;
            pos_setpoint1.type_mask = 0b100111000011;
            pos_setpoint2.type_mask = 0b100111000011;
            pos_setpoint3.type_mask = 0b100111000011;
            ...
            pos_setpoint0.velocity.x = vel_drone_uav0_matlab[0];
            pos_setpoint0.velocity.y = vel_drone_uav0_matlab[1];
            pos_setpoint0.velocity.z = vel_drone_uav0_matlab[2];
            pos_setpoint0.position.z = pos_drone_uav0_matlab[2];

            pos_setpoint1.velocity.x = vel_drone_uav1_matlab[0];
            pos_setpoint1.velocity.y = vel_drone_uav1_matlab[1];
            pos_setpoint1.velocity.z = vel_drone_uav1_matlab[2];
            pos_setpoint1.position.z = pos_drone_uav1_matlab[2];
           

            pos_setpoint2.velocity.x = vel_drone_uav2_matlab[0];
            pos_setpoint2.velocity.y = vel_drone_uav2_matlab[1];
		    pos_setpoint2.velocity.z = vel_drone_uav2_matlab[2];
		    pos_setpoint2.position.z = pos_drone_uav2_matlab[2];

		    pos_setpoint3.velocity.x = vel_drone_uav3_matlab[0];
		    pos_setpoint3.velocity.y = vel_drone_uav3_matlab[1];
		    pos_setpoint3.velocity.z = vel_drone_uav3_matlab[2];
		    pos_setpoint3.position.z = pos_drone_uav3_matlab[2];
...
//20200327byzh
        case command_to_mavros::Takeoff:
           ...
           if(start_flag==0)
             {
              pos_drone_uav0_start=pos_drone_uav0;
              pos_drone_uav1_start=pos_drone_uav1;
              pos_drone_uav2_start=pos_drone_uav2;
              pos_drone_uav3_start=pos_drone_uav3;
              start_flag=1;
            ...
            pos_setpoint0.position.x = pos_drone_uav0_start[0];
            pos_setpoint0.position.y = pos_drone_uav0_start[1];
            pos_setpoint0.position.z = pos_drone_uav0_start[2]+1;

            pos_setpoint1.position.x = pos_drone_uav1_start[0];
            pos_setpoint1.position.y = pos_drone_uav1_start[1];
            pos_setpoint1.position.z = pos_drone_uav1_start[2]+1;

            pos_setpoint2.position.x = pos_drone_uav2_start[0];
            pos_setpoint2.position.y = pos_drone_uav2_start[1];
            pos_setpoint2.position.z = pos_drone_uav2_start[2]+1;

            pos_setpoint3.position.x = pos_drone_uav3_start[0];
            pos_setpoint3.position.y = pos_drone_uav3_start[1];
            pos_setpoint3.position.z = pos_drone_uav3_start[2]+1;
